...
public class StockService {
    ...
    @Autowired
    private StockTransactionRepo transactionRepo;
    @Autowired
    ...
    public void addFromPurchase(Purchase purchase) {
        for (PurchaseItem item : purchase.getItems()) {
            Product product = productRepo.findByName(item.getItemName()).orElseThrow(() -> new CustomException("Product not found: " + item.getItemName(), HttpStatus.NOT_FOUND));

            addStock(product, new BigDecimal(item.getQuantity()), item.getUnitPrice(), "PURCHASE", purchase.getId(), "Purchase: " + purchase.getInvoiceNo());
        }
    }

    // Generic add stock method
    public void addStock(Product product, BigDecimal qty, BigDecimal cost, String refType, Long refId, String note) {
        ...
        BigDecimal newAvgCost = oldValue.add(newValue).divide(newQty, 2, RoundingMode.HALF_UP);
    ...
    public void removeStock(Product product, BigDecimal qty, String refType, Long refId, String note) {
        Stock stock = stockRepo.findByProductId(product.getId()).orElseThrow(() -> new CustomException("Stock not found", HttpStatus.NOT_FOUND));
        ...
        saveTransaction(product, StockType.OUT, qty.negate(), stock.getAverageCost(), newQty, refType, refId, note);
    }

    // Adjust stock (manual correction)
    public void adjustStock(Long productId, BigDecimal newQty, String reason) {
        Product product = productRepo.findById(productId).orElseThrow(() -> new CustomException("Product not found", HttpStatus.NOT_FOUND));
        ...
        saveTransaction(product, StockType.ADJUST, difference, stock.getAverageCost(), newQty, "ADJUSTMENT", null, reason);
    }

    // Get current stock
    public BigDecimal getCurrentStock(Long productId) {
        return stockRepo.findByProductId(productId).map(Stock::getQuantity).orElse(BigDecimal.ZERO);
    }
    ...
    public List<Stock> getLowStockItems() {
        return stockRepo.findAll().stream().filter(s -> s.getAlertQuantity() != null && s.getQuantity().compareTo(s.getAlertQuantity()) <= 0).collect(Collectors.toList());
    }
    ...
    public BigDecimal getStockValue(Long productId) {
        return stockRepo.findByProductId(productId).map(s -> s.getQuantity().multiply(s.getAverageCost())).orElse(BigDecimal.ZERO);
    }

    // Get total inventory value
    public BigDecimal getTotalInventoryValue() {
        return stockRepo.findAll().stream().map(s -> s.getQuantity().multiply(s.getAverageCost())).reduce(BigDecimal.ZERO, BigDecimal::add);
    }

    // Private helper methods
    private Stock getOrCreateStock(Product product) {
        return stockRepo.findByProductId(product.getId()).orElseGet(() -> {
    ...
    private void saveTransaction(Product product, StockType type, BigDecimal qty, BigDecimal cost, BigDecimal balance, String refType, Long refId, String note) {
