...
public class PurchaseExcelExporter {
    ...
    // Colours — same palette as Sales for visual consistency
    ...
    public void export(PurchaseReportDTO report, HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        String filename = "Purchase_Report_" + System.currentTimeMillis() + ".xlsx";
    ...
    private void writeSummarySheet(SXSSFWorkbook wb, XSSFWorkbook xssfWb,
                                    StyleBundle st, PurchaseReportDTO r) {
        ...
        int row = 0;
        row = mergedHeader(sheet, st.header,    row, "RAPID GLOBAL — Purchase Report", 2);
        ...
        row = statCard(sheet, st, row, "Total Cost",     fmt(r.getTotalAmount()),              C_NAVY,   C_LIGHT);
    ...
    // ================================================================
    // SHEET 2 — DETAILS  (streaming — safe at large row counts)
    // ================================================================

    private void writeDetailsSheet(SXSSFWorkbook wb, XSSFWorkbook xssfWb,
                                    StyleBundle st, PurchaseReportDTO r) {
        SXSSFSheet sheet = wb.createSheet("Purchase Details");
        ...
        rowIdx = mergedHeader(sheet, st.header, rowIdx,
                "RAPID GLOBAL — Purchase Detail Report", widths.length);
        rowIdx = mergedHeader(sheet, st.subHeader, rowIdx, dateRangeLabel(r), widths.length);
        rowIdx++;

        List<String> cols = List.of(
                "SL", "Invoice No", "Supplier", "Phone",
                "Purchase Date", "Delivery Date", "Items",
        ...
        for (PurchaseReportRowDTO row : r.getRows()) {
            ...
            cell  (xRow, 2,  row.getSupplierName(),                                                    base);
            cell  (xRow, 3,  row.getPhone(),                                                           base);
            cell  (xRow, 4,  row.getPurchaseDate() != null ? row.getPurchaseDate().format(DATE_FMT) : "", base);
    ...
    private String dateRangeLabel(PurchaseReportDTO r) {
