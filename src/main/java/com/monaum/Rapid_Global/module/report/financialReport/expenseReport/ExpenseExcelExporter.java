...
public class ExpenseExcelExporter {
    ...
    public void export(ExpenseReportDTO report, HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition",
                "attachment; filename=\"Expense_Report_" + System.currentTimeMillis() + ".xlsx\"");
    ...
    private void writeSummarySheet(SXSSFWorkbook wb, XSSFWorkbook xssfWb,
                                    StyleBundle st, ExpenseReportDTO r) {
        SXSSFSheet sheet = wb.createSheet("Summary");
        sheet.setColumnWidth(0, 5500);
        sheet.setColumnWidth(1, 7000);

        int row = 0;
        row = mergedHeader(sheet, st.header,    row, "RAPID GLOBAL — Expense Report", 2);
        ...
        row = statCard(sheet, st, row, "Total Expenses",   fmt(r.getTotalAmount()),              C_RED_FG, C_RED_BG);
    ...
    private void writeDetailsSheet(SXSSFWorkbook wb, XSSFWorkbook xssfWb,
                                    StyleBundle st, ExpenseReportDTO r) {
        SXSSFSheet sheet = wb.createSheet("Expense Details");

        // SL | Expense ID | Category | Payment Method | Txn ID | Paid To | Company | Employee | Invoice | Amount | Date | Description | Status | Approved At | Created By
        int[] widths = {3000, 5000, 5000, 5000, 4500, 5500, 5000, 5000, 4500, 5500, 4500, 6500, 4000, 5500, 5000};
        for (int i = 0; i < widths.length; i++) sheet.setColumnWidth(i, widths[i]);

        int rowIdx = 0;
        rowIdx = mergedHeader(sheet, st.header,    rowIdx, "RAPID GLOBAL — Expense Detail Report", widths.length);
        rowIdx = mergedHeader(sheet, st.subHeader, rowIdx, dateRangeLabel(r),                       widths.length);
        rowIdx++;

        List<String> cols = List.of(
                "SL", "Expense ID", "Category", "Payment Method", "Txn ID",
                "Paid To", "Company", "Employee", "Invoice No",
                "Amount", "Expense Date", "Description",
                "Status", "Approved At", "Created By");
        ...
        for (ExpenseReportRowDTO row : r.getRows()) {
            ...
            cell   (xRow, 1,  row.getExpenseId(),                                                              base);
            cell   (xRow, 2,  row.getCategoryName(),                                                           base);
            cell   (xRow, 3,  row.getPaymentMethod(),                                                          base);
            cell   (xRow, 4,  row.getTransactionId() != null ? row.getTransactionId() : "—",                  base);
            cell   (xRow, 5,  row.getPaidTo() != null ? row.getPaidTo() : "—",                                base);
            cell   (xRow, 6,  row.getPaidToCompany() != null ? row.getPaidToCompany() : "—",                  base);
            cell   (xRow, 7,  row.getEmployeeName() != null ? row.getEmployeeName() : "—",                    base);
            cell   (xRow, 8,  row.getInvoiceNo() != null ? row.getInvoiceNo() : "—",                          base);
            numCell(xRow, 9,  row.getAmount(),                                          st.number(xssfWb, base));
            cell   (xRow, 10, row.getExpenseDate() != null ? row.getExpenseDate().format(DATE_FMT) : "",       base);
            cell   (xRow, 11, row.getDescription() != null ? row.getDescription() : "—",                      base);
            cell   (xRow, 12, row.getStatus() != null ? row.getStatus().name() : "",
                    st.status(xssfWb, row.getStatus()));
            cell   (xRow, 13, row.getApprovedAt() != null ? row.getApprovedAt().format(DT_FMT) : "—",         base);
            cell   (xRow, 14, row.getCreatedByName() != null ? row.getCreatedByName() : "—",                  base);
        ...
        for (int c = 0; c <= 8; c++) cell(total, c, c == 1 ? "TOTALS" : "", st.totalStyle);
        numCell(total, 9, r.getTotalAmount(), st.totalNum(xssfWb));
        for (int c = 10; c < cols.size(); c++) cell(total, c, "", st.totalStyle);
    ...
    private String dateRangeLabel(ExpenseReportDTO r) {
