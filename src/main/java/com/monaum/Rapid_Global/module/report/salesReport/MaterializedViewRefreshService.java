...
public class MaterializedViewRefreshService {
    ...
    public void refreshProductPerformance() {
        try {
            ...
            jdbcTemplate.update(
                "DELETE FROM mv_product_performance WHERE month = DATE_FORMAT(CURDATE(), '%Y-%m')"
            );
            
            // Refresh current month
            jdbcTemplate.update(
                "INSERT INTO mv_product_performance (item_name, month, total_quantity, total_revenue, avg_unit_price, order_count) " +
                "SELECT si.item_name, DATE_FORMAT(s.sell_date, '%Y-%m') as month, " +
                "SUM(si.quantity) as total_quantity, SUM(si.total_price) as total_revenue, " +
                "AVG(si.unit_price) as avg_unit_price, COUNT(DISTINCT si.sales_id) as order_count " +
                "FROM sales_item si JOIN Sales s ON si.sales_id = s.id " +
                "WHERE DATE_FORMAT(s.sell_date, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m') " +
                "AND s.status != 'CANCELLED' " +
                "GROUP BY si.item_name, DATE_FORMAT(s.sell_date, '%Y-%m') " +
                "ON DUPLICATE KEY UPDATE " +
                "total_quantity = VALUES(total_quantity), total_revenue = VALUES(total_revenue), " +
                "avg_unit_price = VALUES(avg_unit_price), order_count = VALUES(order_count)"
            );
    ...
    public void refreshCustomerAnalytics() {
        try {
            ...
            jdbcTemplate.update(
                "INSERT INTO mv_customer_analytics " +
                "(customer_id, customer_name, phone, email, total_orders, total_spent, last_order_date, first_order_date) " +
                "SELECT s.customer_id, s.customer_name, s.phone, s.email, " +
                "COUNT(*) as total_orders, COALESCE(SUM(si.total_price), 0) as total_spent, " +
                "MAX(s.sell_date) as last_order_date, MIN(s.sell_date) as first_order_date " +
                "FROM Sales s LEFT JOIN sales_item si ON s.id = si.sales_id " +
                "WHERE s.status != 'CANCELLED' " +
                "GROUP BY s.customer_id, s.customer_name, s.phone, s.email"
            );
    ...
    public LocalDateTime getLastRefreshTime(String viewName) {
        try {
            String query = String.format(
                "SELECT MAX(updated_at) FROM %s", viewName
            );
